...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/evolutionboy/CokeMine-ServerStatus-Hotaru/master/status.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	wget -N --no-check-certificate "https://raw.githubusercontent.com/evolutionboy/CokeMine-ServerStatus-Hotaru/master/status.sh" && chmod +x status.sh
